...
`QUAST` has been installed using `conda`. We must activate the relavent conda environment before running it.
...
We must now deactivate the `QUAST` `conda` environment. Failure to deactivate the environment will prevent usage of downstream tools.
